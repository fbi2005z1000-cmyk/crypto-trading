...
function normalizeUser(user, username) {
  ...
  if (typeof user.sandboxed !== "boolean") user.sandboxed = false;
  if (typeof user.sandboxReason !== "string") user.sandboxReason = "";
...
function rowToUser(row) {
  ...
  const user = {
    ...
    sandboxReason: row.sandboxReason || "",
...
async function initDb() {
  ...
  if (!names.has("sandboxed")) {
    await dbRun("ALTER TABLE users ADD COLUMN sandboxed INTEGER NOT NULL DEFAULT 0");
  }
  if (!names.has("sandboxReason")) {
    await dbRun("ALTER TABLE users ADD COLUMN sandboxReason TEXT NOT NULL DEFAULT ''");
  }
...
function markUserSandboxed(user, reason = "", adminName = "") {
  if (!user) return;
  user.sandboxed = true;
  const stamp = new Date().toISOString();
  const note = [stamp, adminName || "admin", reason || "manual"].join(" ");
  if (!user.sandboxReason) {
    user.sandboxReason = note;
  } else if (!user.sandboxReason.includes(note)) {
    user.sandboxReason = `${note} | ${user.sandboxReason}`.slice(0, 1000);
  }
}

function calcRiskScore(user) {
  ...
  const profitFactor = loss > 0 ? profit / loss : (profit > 0 ? 2 : 1);
  const ordered = pnlSeries.slice().reverse();
  let running = 0;
  let peak = 0;
  let maxDd = 0;
  ordered.forEach((pnl) => {
  ...
  const equity = Math.max(1, calcUserEquityUsd(user));
  const ddPct = maxDd / equity;
  const highLev = Array.isArray(user?.positions)
    ? user.positions.filter((p) => Number(p?.leverage) >= 20).length
    : 0;
  let score = 60;
  score += (winRate - 0.5) * 40;
  score += Math.min(15, (profitFactor - 1) * 10);
  score -= Math.min(30, ddPct * 100);
  score -= Math.min(15, (Number(user?.strikes) || 0) * 2);
  score -= Math.min(15, highLev * 2);
  score = Math.max(0, Math.min(100, Math.round(score)));
  let label = "Trung bình";
  if (score >= 75) label = "An toàn";
  else if (score < 45) label = "Rủi ro";
  return { score, label };
}
...
function emitRichLeaderboard(targetSocket) {
  const boards = buildRichLeaderboard();
  const payload = { rows: boards.real, rowsReal: boards.real, rowsSandbox: boards.sandbox, ts: Date.now() };
...
io.on("connection", (socket) => {
  ...
  socket.on("admin_action", async (payload) => {
    ...
    if (action === "SEND_MAIL") {
      ...
      const hasReward = !!(
        (Number(reward.usd) || 0) !== 0 ||
        (Number(reward.vnd) || 0) !== 0 ||
        (reward.coins && Object.keys(reward.coins).length) ||
        (reward.boosters && Object.keys(reward.boosters).length) ||
        (Array.isArray(mail?.items) && mail.items.length)
      );
      ...
      recipients.forEach((name) => {
        ...
        if (hasReward) markUserSandboxed(u, "SEND_MAIL", user.username);
        queueSaveUsers(u);
        delivered += 1;
    ...
    if (action === "SET_BALANCE") {
      ...
      await withUserLock(target, async () => {
        ...
        if (updated) {
          markUserSandboxed(targetUser, "SET_BALANCE", user.username);
    ...
    if (action === "CANCEL_ORDER_ADMIN") {
      ...
      await withUserLock(target, async () => {
        ...
        markUserSandboxed(targetUser, "CANCEL_ORDER_ADMIN", user.username);
        queueSaveUsers(targetUser);
        const targetSocket = sockets[target];
        if (targetSocket) emitAccount(targetSocket, targetUser);
    ...
    if (action === "RESET_OPEN_ORDERS") {
      ...
      await withUserLock(target, async () => {
        ...
        markUserSandboxed(targetUser, "RESET_OPEN_ORDERS", user.username);
        queueSaveUsers(targetUser);
        const targetSocket = sockets[target];
        if (targetSocket) emitAccount(targetSocket, targetUser);
    ...
    if (action === "RESET_POSITIONS") {
      ...
      await withUserLock(target, async () => {
        ...
        markUserSandboxed(targetUser, "RESET_POSITIONS", user.username);
        queueSaveUsers(targetUser);
        const targetSocket = sockets[target];
        if (targetSocket) emitAccount(targetSocket, targetUser);
  ...
  socket.on("leaderboard_request", () => {
    ...
    socket.emit("leaderboard_update", {
      rows: boards.real,
      rowsReal: boards.real,
      rowsSandbox: boards.sandbox,
